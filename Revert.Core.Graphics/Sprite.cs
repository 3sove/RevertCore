...
namespace Revert.Core.Graphics
{
    public class Sprite : TextureRegion
    {
        ...
        float[] vertices = new float[SPRITE_SIZE];
        ...
        public static int U3 = 13;
        public static int V3 = 14;
        public static int X4 = 15;
        public static int Y4 = 16;
        ...
        public Sprite(Texture2D texture, int srcWidth, int srcHeight) : this(texture, 0, 0, srcWidth, srcHeight)
        {
        }
        ...
        {
            translate(x - this.position.X, position.Y - this.position.Y);
        }

        /** Sets the position where the sprite will be drawn, relative to its current origin.  */
        public void setOriginBasedPosition(float x, float y)
        {
            setPosition(x - this.originX, position.Y - this.originY);
        }

        /** Sets the position.X position where the sprite will be drawn. If origin, rotation, or scale are changed, it is slightly more efficient
         * to set the position after those operations. If both position and size are to be changed, it is better to use
         * {@link #setBounds(float, float, float, float)}. */
        public void setX(float x)
        {
            translateX(x - this.position.X);
        }

        /** Sets the position.Y position where the sprite will be drawn. If origin, rotation, or scale are changed, it is slightly more efficient
         * to set the position after those operations. If both position and size are to be changed, it is better to use
         * {@link #setBounds(float, float, float, float)}. */
        public void setY(float y)
        {
            translateY(y - this.position.Y);
        }

        /** Sets the position.X position so that it is centered on the given position.X parameter */
        public void setCenterX(float x)
        {
            setX(x - width / 2);
        }

        /** Sets the position.Y position so that it is centered on the given position.Y parameter */
        ...
        /** Sets the position.X position relative to the current position where the sprite will be drawn. If origin, rotation, or scale are
        ...
        /** Sets the position.Y position relative to the current position where the sprite will be drawn. If origin, rotation, or scale are
         * changed, it is slightly more efficient to translate after those operations. */
        ...
        {
            var colorId = GraphicsExtensions.floatToIntColor(packedColor);
            color = new Color((uint)colorId);
        ...
        /** Sets the sprite's scale relative to the current scale. for example: original scale 2 -> sprite.scale(4) -> scale 6.
        ...
        public float[] getVertices()
        {
            if (dirty)
            {
                ...
                if (rotation != 0)
                {
                    ...
                    float cos = (float)Math.Cos(rotationRads);
        ...
        /** Returns the bounding axis aligned {@link Rectangle} that bounds this sprite. The rectangles position.X and position.Y coordinates describe its
        ...
        {
            batch.Draw(texture, position, drawingColor);
        }
        ...
        {
            uint intBits = (uint)GraphicsExtensions.floatToIntColor(vertices[C1]);
            Color color = this.color;
            color.R = (byte)((intBits & 0xff) / 255f);
            color.G = (byte)(((intBits >> 8) & 0xff) / 255f);
            color.B = (byte)(((intBits >> 16) & 0xff) / 255f);
            color.A = (byte)(((intBits >> 24) & 0xff) / 255f);
            return color;
        }
         ...
         * @param position.X the desired horizontal flip state
         * @param position.Y the desired vertical flip state */
         ...
         * @param position.X perform horizontal flip
         * @param position.Y perform vertical flip */
