...
namespace Revert.Core.Mathematics
{
	...
	public String toString()
		...
		{
			return setEulerAnglesRad(yaw * Maths.degreesToRadians, pitch * Maths.degreesToRadians,
				roll * Maths.degreesToRadians);
		}
		...
		public float getRollRad()
		{
			...
			return (float)(pole == 0 ? Math.Atan2(2f * (w * z + y * x), 1f - 2f * (x * x + z * z))
				: pole * 2f * Math.Atan2(y, w));
		}
		...
		{
			int pole = getGimbalPole();
			return (float)(pole == 0 ? (float)Math.Asin(Maths.clamp(2f * (w * x - z * y), -1f, 1f)) : (float)pole * Math.PI * 0.5f);
		}
		...
		{
			float len = len2();
			if (len != 0.0f && !Maths.isEqual(len, 1f))
			{
		...
		{
			return Maths.isZero(x, tolerance) && Maths.isZero(y, tolerance) && Maths.isZero(z, tolerance)
				&& Maths.isEqual(w, 1f, tolerance);
		}
		...
		//{
		//	return setFromAxes(normalizeAxes, matrix.Value[Matrix3.M00], matrix.val[Matrix3.M01], matrix.val[Matrix3.M02],
		...
		{
			float dot = Maths.clamp(v1.dot(v2), -1f, 1f);
		...
		public Quaternion setFromCross(float x1, float y1, float z1, float x2, float y2,
			float z2)
		{
			float dot = Maths.clamp(Vector3.dot(x1, y1, z1, x2, y2, z2), -1f, 1f);
			float angle = (float)Math.Acos(dot);
	...
	public bool equals(Object obj)
		{
			...
			return (w.GetHashCode() == other.w.GetHashCode())
				&& (x.GetHashCode() == other.x.GetHashCode())
				&& (y.GetHashCode() == other.y.GetHashCode())
				&& (z.GetHashCode() == other.z.GetHashCode());
		}
		...
		public float getAxisAngleRad(Vector3 axis)
		{
			...
			if (s < Maths.FLOAT_ROUNDING_ERROR)
		...
		public float getAngleAroundRad(float axisX, float axisY, float axisZ)
		{
			...
			return Maths.isZero(l2) ? 0f
				: (float)(2.0 * Math.Acos(Maths.clamp((float)((d < 0 ? -this.w : this.w) / Math.Sqrt(l2)), -1f, 1f)));
		}
