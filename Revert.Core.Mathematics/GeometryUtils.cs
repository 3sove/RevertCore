...
namespace Revert.Core.Mathematics
{
    ...
    {
        private static Vector2 tmp1 = new Vector2(0f, 0f);
        private static Vector2 tmp2 = new Vector2(0f, 0f);
        private static Vector2 tmp3 = new Vector2(0f, 0f);
        ...
        static public bool colinear(float x1, float y1, float x2, float y2, float x3, float y3)
        {
            ...
            return Math.Abs(det) < Maths.FLOAT_ROUNDING_ERROR;
        }
        ...
        static public Vector2 triangleCircumcenter(float x1, float y1, float x2, float y2, float x3, float y3, Vector2 circumcenter)
        {
            ...
            float det = dx32 * dy21 - dx21 * dy32;
            if (Math.Abs(det) < Maths.FLOAT_ROUNDING_ERROR)
        ...
        static public float triangleCircumradius(float x1, float y1, float x2, float y2, float x3, float y3)
        {
            ...
            if (Math.Abs(y2 - y1) < Maths.FLOAT_ROUNDING_ERROR)
            {
                m2 = -(x3 - x2) / (y3 - y2);
                mx2 = (x2 + x3) / 2;
                my2 = (y2 + y3) / 2;
            ...
            else if (Math.Abs(y3 - y2) < Maths.FLOAT_ROUNDING_ERROR)
            {
                m1 = -(x2 - x1) / (y2 - y1);
                mx1 = (x1 + x2) / 2;
                my1 = (y1 + y2) / 2;
        ...
        static public bool isClockwise(Vector2[] polygon, int offset, int count)
        {
