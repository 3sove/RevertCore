...
namespace Revert.Core.GuitarProReader.GuitarPro.VersionSpecific
{
    internal class Gp5InputStream : GpInputStream
    {
        ...
        internal static GuitarProFile Create(Stream stream, Version version)
        {
            var inputStream = new Gp5InputStream(stream) {Version = version};
            var header = inputStream.ReadHeader(version);

            return new GuitarProFile
            {
                Header = header,
                Body = inputStream.ReadBody(header)
            };
        }
        ...
        {
            var info = new AddInfo {Tempo = ReadInt()};
            if (Version.Minor != "00") stream.Seek(1, SeekOrigin.Current);
            /*info.Key =*/
            ReadInt();
            /*info.Octave =*/
            ReadByte();
        ...
        protected Body ReadBody(Header header)
        {
            ...
            var pairs = new List<MeasureTrackPair>();
        ...
        protected GraceNote ReadGraceNote()
        {
            ...
            var graceNote = new GraceNote {Fret = ReadByte()};
            //TODO
            ReadByte(); //Remove this when dynamic is finished
            //var dynamic = (Dynamic)Enum.ToObject(typeof(Dynamic), ReadByte());
            graceNote.Dynamic = Dynamic.Invalid;
            //TODO
            ReadByte(); //Remove this when transition is finished
