...
namespace Revert.Core.Text.Tokenization.Porter2
{
    public class EnglishWord
    {
        internal static EnglishWord CreateWithR1R2(string text)
        {
            ...
            result.r1 = CalculateR(result.Stem, 0);
            result.r2 = CalculateR(result.Stem, result.r1.Start);
            return result;
        }
        ...
        private WordRegion r1;
        private WordRegion r2;

        private bool cachedUpToDate;
        ...
        {
            if (cachedUpToDate == false)
            {
                r1.GenerateRegion(stem);
                r2.GenerateRegion(stem);
                cachedUpToDate = true;
            }
        }
        ...
        internal void GenerateStem()
        {
            ...
            {
                r1 = CalculateR(Stem, 2);
            }
            else if (Stem.StartsWith("commun"))
            {
                r1 = CalculateR(Stem, 3);
            }
            else
            {
                r1 = CalculateR(Stem, 0);
            }
            r2 = CalculateR(Stem, r1.Start);
        ...
        internal bool IsShortSyllable(int index)
        {
            ...
            if (vowelIndex > 0)
            {
                var expectedShortEnd = vowelIndex + 2; //Check word has room for a single non-vowel 
        ...
        private void StripEnding(IEnumerable<string> endings)
        {
