    ...
    public class NeuralNetwork
    {
        ...
        private void InitBiases()//initializes random array for the biases being held within the network.
        {
            ...
            for (int i = 1; i < layers.Length; i++)
            {
                ...
                {
                    bias[j] = Maths.randomFloat(-0.5f, 0.5f);
                }
                biasList.Add(bias);
            }
        ...
        private void InitWeights()//initializes random array for the weights being held in the network.
        {
            ...
            for (int i = 1; i < layers.Length; i++)
            {
                ...
                for (int j = 0; j < layers[i]; j++)
                {
                    ...
                    {
                        neuronWeights[k] = Maths.randomFloat(-0.5f, 0.5f);
                    }
        ...
        public float[] FeedForward(float[] inputs)//feed forward, inputs >==> outputs.
        {
            ...
            for (int i = 1; i < layers.Length; i++)
            {
                for (int j = 0; j < layers[i]; j++) //j = current layer index
                {
                    float value = 0f;
                    for (int k = 0; k < layers[i - 1]; k++) //k = previous layer index
                    {
                        value += weights[i - 1][j][k] * neurons[i - 1][k]; //layer->current layer neuron->previous layer neuron    WEIGHT * VALUE    previous layer->previous layer neuron
                    }
                    neurons[i][j] = activate(value + biases[i - 1][j], i - 1);  //current layer.current layer neuron   VALUE = activate(summed values + CURRENT layer->current layer neuron BIAS)
                }
            }
        ...
        public void BackPropagate(float[] inputs, float[] expected)//backpropogation;
        {
            ...
            for (int i = layers.Length - 2; i > 0; i--)//runs on all hidden layers
            {
                for (int j = 0; j < layers[i]; j++)//outputs
                {
                    ...
                    gamma[i][j] *= activateDer(neurons[i][j], i-1);//calculate gamma
                }
        ...
        public void Mutate(int high, float val)//used as a simple mutation function for any genetic implementations.
        {
            for (int i = 0; i < biases.Length; i++)
            {
                ...
                {
                    biases[i][j] = (Maths.randomFloat(0f, high) <= 2) ? biases[i][j] += Maths.randomFloat(-val, val) : biases[i][j];
                }
            }
            ...
            for (int i = 0; i < weights.Length; i++)
            {
                for (int j = 0; j < weights[i].Length; j++)
                {
                    ...
                    {
                        weights[i][j][k] = (Maths.randomFloat(0f, high) <= 2) ? weights[i][j][k] += Maths.randomFloat(-val, val) : weights[i][j][k];
                    }
                }
            }
        }
